...
def get_action_prompt(instruction='', summary_history=[], action_history=[], last_summary='', last_action='',
    ...
    prompt = "### Background ###\n"
    prompt += "You are concentrating on helping the people in the office to complete their instructions by providing their needed items to them or delivering some items from them to other people."
    ...
    prompt += "The locker is locked and the QR code can help people to unlock the locker and acquire the item in the locker. People don't have a QR code in the initial situation, you need to send a QR code, but you should avoid sending back-to-back QR code.\n"
    ...
    prompt += "Send QR code(contact, item description): Send \"contact\" a QR code and the \"item\"'s description, which the QR Code is used to unlock the locker on your physical body. You can use it to send QR code for person. It is a core action to ensure the person can get or put the item in the locker.\n"
    ...
    prompt += "Wait in place(user): The Physical body waits at the current location for the \"user\" to scanned QR code and received the item and put the item in the locker at that time. \"Locker information\" will tell you whether he has done the task of what you asked of him in his location. \n"

    prompt += "### Here are generalized actions ###"
    ...
    prompt += "You should wait until the current person complete their current task which can be found in \"locker information\" and then you can move to the next location.\n"
    prompt += "You need to send a QR code to a person who involved in the current action of the delivered item, you should use it before \"Wait\", you can find whether you have sent a QR code of that delivered item in the action history.\n"
    # prompt += "When your physical body moves to someone's position for a task, you need to \"inform\" that person what you need them to do.\n"
    # prompt += "Only after you send the QR code, the person in the corresponding location can scan QR code open the locker and take or put in the corresponding items.\n"
    # prompt += "Each time you \"move\" to a location, the person of the location need a QR code to open the locker, received the item and put the item in the locker, in this occation you need to use \"Send QR code\".\n"
    ...
    prompt += "Before you forward electronic file to someone, you need to check the conversations history for whether you have recevied an electronic file or not. If you do not recive an electronic file in the Conversations history, you need to ask \"user\" to send an electronic file to you.\n"
    ...
    # prompt += "Virtual action and physical body action do not always occur at the same time.\n"


    prompt += "### Response requirements ###\n"
    prompt += "For actions, you must select them from Virtual WeChat Operations, Physical body Controls actions and generalized actions.\n"
    # prompt += "Please note that Virtual WeChat Operations and Physical body Controls can be executed simultaneously. \n"
    prompt += "- Ensure that each action command is clear, specific, and adheres to the above limitations.\n"
    prompt += "You can judge whether the person complete his sub-tasks by finding it in\"Locker information\".\n"

    # prompt += "You need to make sure the person involved in the current subtask know his task by \"inform\" and the person involved in the task can accurately receive and place the items to be delivered by \"Send QR code\" , \"Move\" and \"Wait\" and finally \"Stop\".\n"
    ...
    prompt += "### Action ###\nVirtual actions and physical body actions can be output simultaneously. Make sure that the parameters in the \"()\". Such as Move(A).  \n"
...
def get_reflect_prompt(instruction='', last_thought='', last_action='', last_description='', current_thought='',
                       ...
                       user_chat_history='', group_chat_history=''):
...
def get_planning_prompt(instruction='', thought_history=[], action_history=[], last_summary='', last_action='',
    ...
    # prompt += f"The physical robots can only move and wait in place, further operations require human assistance.\n"

    prompt += "### Distribution ###\n"
    prompt += "The original distribution of items in the office scene:\n"
    prompt += priori_knowledge
    ...
    prompt += "### Output format ###\n"
    prompt += "Your output format is:\n"
    prompt += "### Items location ###\nOnly give the current location of the involved items. Example：glue is in the location of 李永长\n"
